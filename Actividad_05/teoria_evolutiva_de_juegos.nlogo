...
to init_pagos
  ...
  set colores map [ k -> 15 + 30 * k ] (range num_estrategias) ;; 15 color rojo
end
...
to colorear_jugadores
  ask turtles [ set color item estrategia colores]
end
...
to actualizar_grafica
  ...
  let frec_estrategias map [ n ->
    (count turtles with [estrategia = n]) / (count turtles)] nums_estrategias
  set-current-plot "Frecuencia estrategias"
  foreach nums_estrategias [
    n -> set-current-plot-pen (word n)
