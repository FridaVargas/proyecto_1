...
to go
  ...
  ask one-of patches [
    ; elegir a un competidor
    let mi_competidor one-of other patches
    if tipo_interaccion = "local" [
      set mi_competidor one-of neighbors
    ]

    ; revisar si gano o no
    ifelse le_gano? mi_competidor [
      ;; si sí le gano, entonces lo invado
      ask mi_competidor [set pcolor [pcolor] of myself]
    ][
      ;; revisar si mi competidor me gana
      if [le_gano? myself] of mi_competidor [
        ;; si me gana me invade
        set pcolor [pcolor] of mi_competidor
      ]
    ]
  ]
...
"pen-1" 1.0 0 -14070903 true "" "plot count patches with [pcolor = blue ]"
